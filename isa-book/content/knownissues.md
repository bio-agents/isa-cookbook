...
isaagents v0.9 package
---------------------

-   Various API packages have been refactored and moved.
    `isaagents.model.v1` is now in `isaagents.model`. Packages
    `isaagents.validate`, `isaagents.config`, `isaagents.schemas` have been
    removed. Some items from `isaagents.convert` and `isaagents.io` have
    been split into the `isaagents.io` or `isaagents.net` packages, where
...
-   New ISA creation functionality from planning objects in
    `isaagents.create.models` currently only support a limited number of
...
-   For certain of ISA-Tab table files, the ISA-Tab parser cannot
    ...
    in correcting them, in the `isaagents.utils` package. \#146 (see
...
tracker](https://github.com/ISA-agents/isa-api/issues).
